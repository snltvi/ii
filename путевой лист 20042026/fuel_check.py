...
# -*- coding: utf-8 -*-
"""
Показання датчиків рівня палива для кожного авто на початок (01.04.2026) та кінець (14.04.2026) рейсу.
...
# Ключові слова для вибору датчиків рівня палива
FUEL_KEYWORDS = [
    "уровень топлива",
    "датчик топлива",
...
def sensor_is_fuel(name: str) -> bool:
    lo = name.lower().strip()
    return any(kw in lo for kw in FUEL_KEYWORDS)
...
# ── Filter fuel sensors for each vehicle ──────────────────────────────────
# fuel_sensors[oid] = [ {sid, pid, name, slist_param}, ... ]
fuel_sensors = {}
for oid_str, vdata in vehicles_cache.items():
    found = []
    for s in vdata.get("sensors", []):
        if sensor_is_fuel(s.get("name", "")):
            sid_val = s.get("sid", 0)
            pid_val = s.get("pid", 0)
            if sid_val > 0:
                slist = f"s{sid_val}"
...
for obj in objects:
    oid_str = str(obj["id"])
    sensors = fuel_sensors.get(oid_str, [])
    readings[oid_str] = {}
...
def flit(v):
    if v is None: return "—"
    try: return f"{float(v):>12,.1f}".replace(",", " ")
    except: return str(v)

W = 110
print("=" * W)
print("  ПОКАЗАННЯ ДАТЧИКІВ РІВНЯ ПАЛИВА (БАК)")
...
for gid in GROUP_ORDER + [g for g in {o.get("groupId") for o in objects} if g not in GROUP_ORDER]:
    ...
    print(f"  {'Авто':<20} {'ID':<6} {'Датчик':<36} "
          f"{'ID-тип':<12} "
          f"{'01.04 (початок)':>16}  {'14.04 (кінець)':>16}  Статус")
    print("  " + "-" * (W - 2))
    ...
    for obj in group_objs:
        ...
        if not sensors:
            print(f"  {obj['name']:<20} {obj['id']:<6} "
                  f"{'⚠ датчик палива не знайдено в кеші':<36}")
            continue
        ...
        for s in sensors:
            ...
            print(f"  {name_col:<20} {id_col:<6} {s['name']:<36} "
                  f"{s['id_type']:<12} "
                  f"{flit(v_s):>16}  {flit(v_e):>16}  {status_str}")
        ...
        if len(sensors) > 1:
            for s in sensors:
                ...
                if v_s is not None and v_e is not None:
                    diff = v_e - v_s
                    sign = "+" if diff >= 0 else ""
                    print(f"  {'':20} {'':6} {'  → витрата/зміна за рейс:':36} "
                          f"{'':12} {'':>16}  {sign}{flit(diff):>15} л")
        print()
