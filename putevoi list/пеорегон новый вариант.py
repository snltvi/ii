...
LOGIN = "abvprom"
PASSWORD = "29328"
# Путь к файлу, где есть колонки "ID объекта" и "Бак"
INPUT_FILE = r"c:/Users/snltv/Desktop/ii/putevoi list/Cцепка_водитель-авто-прицеп_на_20_01_2026_с_ID_объектов.xlsx"
...
def main():
    ...
    # Загружаем Excel. Если у вас CSV, замените на pd.read_csv
    ...
    # Создаем словари для быстрого поиска Номера и Бака по ID
    car_names = pd.Series(df_input['Номер авто'].values, index=df_input['ID объекта'].astype(int)).to_dict()
    # Берем объем бака из Excel. Если в ячейке пусто, ставим 600 по умолчанию
    tank_map = pd.Series(df_input['Бак'].values, index=df_input['ID объекта'].astype(int)).fillna(600).to_dict()
    ...
    while curr <= end_dt:
        ...
        for oid in ids_list:
            ...
            vol = int(tank_map.get(oid, 600))

            # ПРОВЕРКА: Есть ли уже запись в базе?
            conn = sqlite3.connect(DB_NAME)
            cursor = conn.cursor()
            cursor.execute("SELECT 1 FROM abv_fuel_in_out_comsum WHERE obj_id = ? AND report_date = ?", (oid, d_str))
            exists = cursor.fetchone()
            conn.close()

            if exists:
                print(f"⏩ {v_name}: Пропуск (уже в базе)")
                continue
                ...
                if res and len(res) > 0:
                    ...
                    if obj.get('sensors'):
                        for s in obj['sensors']:
                            s_name = s.get('sensor_name', '').lower()
                            if any(w in s_name for w in ["бак", "lls", "fuel", "датчик"]):
        ...
        if day_data:
            save_to_db(day_data)
            print(f"💾 Данные за {d_str} сохранены.")
    ...
    if os.path.exists(DB_NAME):
        ...
        conn.close()
        report_name = "Full_History_Report.html"
        df_final.to_html(report_name, index=False)
