...
TZ_UA = timedelta(hours=3)   # Україна UTC+3 — для конвертації в UTC при запитах до API
...
def get_track(sid, oid, dt_from, dt_to):
    """
    /track приймає UTC, повертає coords[{tm,lat,lon,dir,dst,st,speed}].
    Конвертуємо локальний час UA → UTC для запиту.
    Нормалізуємо поле часу: tm → dt, щоб решта коду не змінювалась.
    """
    try:
        utc_from = ds(dp(dt_from) - TZ_UA)
        utc_to   = ds(dp(dt_to)   - TZ_UA)
        ...
        if r.status_code == 200:
            ...
            # tm вже в локальному часі UA (session timezone=3)
            # просто нормалізуємо формат у поле dt
            for pt in coords:
                if pt.get('tm') and not pt.get('dt'):
    ...
    try:
        ...
        r = requests.get(f"{API_URL}/objdata",
                         ...
                         params={
                             'oid': oid,
                             'slist': f's{sensor_id}',
                             'from': ds(target - timedelta(hours=window_h) - TZ_UA),
                             'to':   ds(target + timedelta(hours=window_h) - TZ_UA),
                         }, timeout=30)
...
def get_fuelings_period(sid, oid, dt_from, dt_to):
    ...
    try:
        r = requests.get(f"{API_URL}/fuelings",
                         headers={'SessionId': sid},
                         params={'oid': oid,
                                 'from': ds(dp(dt_from) - TZ_UA),
                                 'to':   ds(dp(dt_to)   - TZ_UA)},
                         timeout=30)
...
def get_first_movement(sid, oid, dt_from, search_hours=14):
    """
    Находит первую точку с speed > 0 начиная с dt_from в пределах search_hours.
    Возвращает dict {'dt', 'lat', 'lon', 'speed'} или None.
    """
    dt_start = dp(dt_from)
    dt_end   = ds(dt_start + timedelta(hours=search_hours))
    track    = get_track(sid, oid, dt_from, dt_end)
    if not track:
        return None
    for pt in track:
        spd = pt.get('speed', 0) or 0
        if spd > 0 and pt.get('lat') and pt.get('lon') and pt.get('dt'):
            return pt
    return None


def get_last_movement(sid, oid, dt_to, search_hours=14):
    """
    Находит последнюю точку с speed > 0 до dt_to в пределах search_hours назад.
    Возвращает dict {'dt', 'lat', 'lon', 'speed'} или None.
    """
    dt_end   = dp(dt_to)
    dt_start = ds(dt_end - timedelta(hours=search_hours))
    track    = get_track(sid, oid, dt_start, dt_to)
    if not track:
        return None
...
def get_fuel_at_time(sid, oid, target_str, window_h=1):
    """
    Возвращает уровень топлива на момент target_str через getobjectsfuelinfo
    с узким окном ± window_h.  Суммирует датчики бак/lls/fuel.
    """
    try:
        target  = dp(target_str)
        dt_from = ds(target - timedelta(hours=window_h) - TZ_UA)
        dt_to   = ds(target + timedelta(hours=window_h) - TZ_UA)
        ...
        if not found:
            for s in obj.get('sensors', []):
                total += s.get('beginLevel', 0) or 0
        return round(total, 1) if total > 0 else None
    except Exception as e:
        print(f"  Ошибка get_fuel_at_time: {e}")
        return None
...
def get_fuel_levels(sid, oid, dt_from, dt_to):
    ...
    try:
        r = requests.get(f"{API_URL}/getobjectsfuelinfo",
                         headers={'SessionId': sid},
                         params={'objuids': str(oid),
                                 'date_from': ds(dp(dt_from) - TZ_UA),
                                 'date_to':   ds(dp(dt_to)   - TZ_UA)},
                         timeout=30)
        if r.status_code != 200:
...
def page_step2(query):
    ...
    fm_time = None   # время первого движения
    fm_fuel = None   # уровень бака при первом движении
    fm_odo  = None   # одометр при первом движении
    fm_addr = ""     # адрес при первом движении
    lm_time = None   # время последнего движения
    lm_fuel = None   # уровень бака при последнем движении
    lm_odo  = None   # одометр при последнем движении
    lm_addr = ""     # адрес при последнем движении

    if sid_api:
        # ── Первое движение ──
        print(f"  Ищу первое движение с {dt1}...")
        fm_pt = get_first_movement(sid_api, oid, dt1)
        if fm_pt:
            fm_time = fm_pt['dt']
            print(f"  → Первое движение: {fm_time}  (speed={fm_pt.get('speed')} км/ч)")
            if sid_s:
                fm_odo, _ = get_odometer_at_time(sid_api, oid, sid_s, fm_time, window_h=1)
                print(f"  → Одометр при старте: {fm_odo} км")
            fm_fuel = get_fuel_at_time(sid_api, oid, fm_time)
            print(f"  → Бак при старте: {fm_fuel} л")
            _, _, fm_addr = get_location_at_time(sid_api, oid, fm_time)
            print(f"  → Адрес старта: {fm_addr}")
        else:
            print("  → Движение не обнаружено за 14 ч от dt1")

        # ── Последнее движение ──
        print(f"  Ищу последнее движение до {dt2}...")
        lm_pt = get_last_movement(sid_api, oid, dt2)
        if lm_pt:
            lm_time = lm_pt['dt']
            print(f"  → Последнее движение: {lm_time}  (speed={lm_pt.get('speed')} км/ч)")
            if sid_s:
                lm_odo, _ = get_odometer_at_time(sid_api, oid, sid_s, lm_time, window_h=1)
                print(f"  → Одометр при финише: {lm_odo} км")
            lm_fuel = get_fuel_at_time(sid_api, oid, lm_time)
            print(f"  → Бак при финише: {lm_fuel} л")
            _, _, lm_addr = get_location_at_time(sid_api, oid, lm_time)
            print(f"  → Адрес финиша: {lm_addr}")
        else:
            print("  → Движение не обнаружено за 14 ч до dt2")

    if sid_api and sid_s:
        lookup_dt1 = fm_time if fm_time else dt1
        print(f"  Ищу одометр начала на {lookup_dt1}...")
        odo1_val, odo1_time = get_odometer_at_time(sid_api, oid, sid_s, lookup_dt1)
        print(f"  → {odo1_val} км ({odo1_time})" if odo1_val else "  → не найдено")

        lookup_dt2 = lm_time if lm_time else dt2
        print(f"  Ищу одометр конца на {lookup_dt2}...")
        odo2_val, odo2_time = get_odometer_at_time(sid_api, oid, sid_s, lookup_dt2)
    ...
    def movement_block(label, mt, modo, mfuel, maddr, color):
        if not mt:
            return ""
        odo_s  = f"{modo:,} км" if modo else "не определён"
        fuel_s = f"{mfuel} л"   if mfuel is not None else "не определён"
        addr_s = esc(maddr) if maddr else "—"
        return f"""
        <div class="ok-box" style="margin-bottom:10px; border-left-color:{color}">
          <strong>{label}: {esc(mt)}</strong><br>
          📍 {addr_s}<br>
          Одометр: <strong>{odo_s}</strong> &nbsp;|&nbsp; Бак: <strong>{fuel_s}</strong>
        </div>"""

    fm_block = movement_block("🚦 Первое движение", fm_time, fm_odo, fm_fuel, fm_addr, "#4caf50")
    lm_block = movement_block("🏁 Последнее движение", lm_time, lm_odo, lm_fuel, lm_addr, "#e53935")
...
<div class="card">
  ...
  <form action="/report" method="get" id="frm2"
    ...
    <input type="hidden" name="fm_time" value="{esc(fm_time or '')}">
    <input type="hidden" name="fm_fuel" value="{fm_fuel if fm_fuel is not None else ''}">
    <input type="hidden" name="fm_addr" value="{esc(fm_addr)}">
    <input type="hidden" name="lm_time" value="{esc(lm_time or '')}">
    <input type="hidden" name="lm_fuel" value="{lm_fuel if lm_fuel is not None else ''}">
    <input type="hidden" name="lm_addr" value="{esc(lm_addr)}">
...
def page_report(query):
    ...
    fm_time  = query.get('fm_time', [''])[0] or None
    fm_fuel  = float(query.get('fm_fuel', [''])[0]) if query.get('fm_fuel', [''])[0] else None
    fm_addr  = query.get('fm_addr', [''])[0] or None
    lm_time  = query.get('lm_time', [''])[0] or None
    lm_fuel  = float(query.get('lm_fuel', [''])[0]) if query.get('lm_fuel', [''])[0] else None
    lm_addr  = query.get('lm_addr', [''])[0] or None
    ...
    if fm_time:
        print(f"  Первое движение: {fm_time}  |  Бак при старте: {fm_fuel} л")
    ...
    if fm_addr:
        lat1, lon1, addr1 = None, None, fm_addr
        print(f"\n  Адрес старта (из шага 2): {addr1}")
    else:
        print("\n  Определяю начальную точку...")
        lat1, lon1, addr1 = get_location_at_time(sid_api, oid, fm_time or dt1)
        print(f"  → {addr1}")

    if lm_addr:
        lat2, lon2, addr2 = None, None, lm_addr
        print(f"  Адрес финиша (из шага 2): {addr2}")
    else:
        print("  Определяю конечную точку...")
        lat2, lon2, addr2 = get_location_at_time(sid_api, oid, lm_time or dt2)
        print(f"  → {addr2}")
    ...
    fuel_begin_api, fuel_end_api = get_fuel_levels(sid_api, oid, dt1, dt2)

    if fm_fuel is not None:
        fuel_begin = fm_fuel
        print(f"  → начало (первое движение {fm_time}): {fuel_begin} л")
    else:
        fuel_begin = fuel_begin_api
        print(f"  → начало: {fuel_begin} л")

    if lm_fuel is not None:
        fuel_end = lm_fuel
        print(f"  → конец (последнее движение {lm_time}): {fuel_end} л")
    else:
        fuel_end = fuel_end_api
        print(f"  → конец: {fuel_end} л")
