...
ЗАПРАВКИ из базы данных SQLite
"""
...
DB_FILE = os.path.join(SCRIPT_DIR, "abv_fuel_in_out_comsum-16-02-206.db")
PORT = 8080
...
def get_fuel_from_db(oid, date_from, date_to):
    """
    НОВОЕ: Получение топлива ИЗ БАЗЫ ДАННЫХ
    Returns: (fuel_start, fuel_end, total_refuels)
    """
    print(f"\n{'='*60}")
    print("ПОЛУЧЕНИЕ ДАННЫХ ПО ТОПЛИВУ ИЗ БД")
    print(f"{'='*60}")
    print(f"   📊 База данных: {os.path.basename(DB_FILE)}")
    ...
    try:
        ...
        query = """
            SELECT report_date, begin_level, refuels, end_level, consumption
        ...
        if not rows:
            print(f"   ⚠️ Нет данных в БД за этот период")
            return 0, 0, 0
        
        # Берём ПЕРВЫЙ день = начальный уровень
        first_day = rows[0]
        fuel_start = first_day[1]  # begin_level
        
        # Берём ПОСЛЕДНИЙ день = конечный уровень
        last_day = rows[-1]
        fuel_end = last_day[3]  # end_level
        
        # Суммируем ВСЕ заправки за период
        total_refuels = sum(row[2] for row in rows)  # refuels
        
        print(f"\n   📊 ДАННЫЕ ИЗ БД:")
        print(f"      Дней с данными: {len(rows)}")
        print(f"      Топливо начало ({rows[0][0]}): {fuel_start} л")
        print(f"      Топливо конец ({rows[-1][0]}):  {fuel_end} л")
        print(f"      Заправлено (сумма):  {total_refuels} л")
        
        # Показываем детали по дням если их мало
        if len(rows) <= 7:
            print(f"\n   📋 ДЕТАЛИ ПО ДНЯМ:")
            for row in rows:
                print(f"      {row[0]}: начало={row[1]}л, заправки={row[2]}л, конец={row[3]}л, расход={row[4]}л")
        
        print(f"{'='*60}\n")
        
        return round(fuel_start, 1), round(fuel_end, 1), round(total_refuels, 1)
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        <head>
            <meta charset="UTF-8">
            <title>Путевой лист (с БД заправок)</title>
            <style>
                ...
                .badge {{ display: inline-block; background: #28a745; color: white; padding: 2px 8px; border-radius: 3px; font-size: 11px; margin-left: 10px; }}
            </style>
        </head>
        <body>
            <div class="card">
                <h2>📋 Путевой Лист <span class="badge">БД заправок</span></h2>
    ...
    def do_POST(self):
        ...
        # НОВОЕ: Получаем топливо ИЗ БД
        fuel_start, fuel_end, fuelings = get_fuel_from_db(int(car['ID объекта']), date_1, date_2)
        ...
        <head>
            ...
            <style>
                ...
                .db-badge {{ background: #28a745; color: white; padding: 3px 8px; border-radius: 3px; font-size: 11px; }}
                ...
                @media print {{ .buttons, .db-badge {{ display: none; }} }}
            </style>
        </head>
        <body>
            <div class="bill">
                <h1>ПУТЕВОЙ ЛИСТ <span class="db-badge">БД</span></h1>
                ...
                <table>
                    <tbody>
                        ...
                        <tr>
                            <td><strong>Заправлено <span class="db-badge">из БД</span></strong></td>
...
def main():
    ...
    print("\n" + "="*60)
    print("ПУТЕВОЙ ЛИСТ С ЗАПРАВКАМИ ИЗ БД")
    print("="*60)
    ...
    if not os.path.exists(DB_FILE):
        print(f"\n⚠️ База данных {DB_FILE} не найдена!")
        print("   Заправки будут = 0")
    else:
        print(f"\n✓ База данных найдена: {os.path.basename(DB_FILE)}")
