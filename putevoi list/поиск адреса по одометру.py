...
def get_refuels_from_db(oid, date_from, date_to):
    """
    НОВОЕ: Получение ТОЛЬКО ЗАПРАВОК из базы данных
    Returns: float (литры)
    """
    print(f"\n{'='*60}")
    print("ПОЛУЧЕНИЕ ЗАПРАВОК ИЗ БД")
    print(f"{'='*60}")
    ...
    try:
        ...
        # Получаем только заправки за период
        query = """
        ...
        if len(rows) <= 10:
            print(f"\n   📋 ЗАПРАВКИ ПО ДНЯМ:")
            for row in rows:
                if row[1] > 0:  # Показываем только дни с заправками
                    print(f"      {row[0]}: {row[1]} л")
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        print(f"\nВЫЕЗД:")
        print(f"  Время:          {p1['dt']}")
        print(f"  Одометр:        {p1['odo']:.1f} км")
        print(f"  Координаты:     {p1.get('lat', 'N/A')}, {p1.get('lon', 'N/A')}")
        print(f"  Адрес:          {p1['addr']}")
        print(f"\nВОЗВРАТ:")
        print(f"  Время:          {p2['dt']}")
        print(f"  Одометр:        {p2['odo']:.1f} км")
        print(f"  Координаты:     {p2.get('lat', 'N/A')}, {p2.get('lon', 'N/A')}")
        print(f"  Адрес:          {p2['addr']}")
        print(f"\nТОПЛИВО:")
        print(f"  Начало:         {fuel_start} л")
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{p1.get('lat', 'N/A')}, {p1.get('lon', 'N/A')}</td>
                            <td>{p2.get('lat', 'N/A')}, {p2.get('lon', 'N/A')}</td>
                        </tr>
                        <tr>
