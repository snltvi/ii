...
def main():
    ...
    try:
        ...
        print(f"✓ Получены уровни топлива по {len(fuel_data)} ТС")
    
    except Exception as e:
        print(f"❌ Ошибка запроса уровней: {e}")
        input("\nНажмите Enter...")
        return
    
    # 2. Заправки за период
    print(f"⛽ Запрос заправок...")
    fuelings_map = {}  # {oid: total_fuelings}
    
    for idx, oid in enumerate(ids_list, 1):
        oid = int(oid)
        print(f"\r   Заправки: {idx}/{len(ids_list)}...", end='', flush=True)
        
        try:
            fuelings_res = requests.get(
                f"{API_BASE_URL}/fuelings",
                params={'oid': oid, 'from': d_start, 'to': d_end},
                headers={'SessionId': sid},
                timeout=15
            )
            
            if fuelings_res.status_code == 200:
                data = fuelings_res.json()
                ...
                if data.get('result') == 'Ok':
                    ...
                    fuelings_map[oid] = round(total, 1)
                else:
                    fuelings_map[oid] = 0
            else:
                fuelings_map[oid] = 0
    ...
    # Показать найденные заправки
    total_fuelings_found = sum(1 for v in fuelings_map.values() if v > 0)
    total_fuelings_volume = sum(fuelings_map.values())
    print(f"   Найдено заправок: {total_fuelings_found} ТС, {total_fuelings_volume:.1f} л всего")
    ...
    for obj in fuel_data:
        ...
        # Заправки за период
        fuelings = fuelings_map.get(oid, 0)
        
        # Расход = Начало + Заправки - Конец
        consumption = t_start + fuelings - t_end
        ...
        results.append({
            ...
            'Заправлено (л)': fuelings,
            'Топливо конец (л)': round(t_end, 1),
            'Расход (л)': round(consumption, 1),
    ...
    for _, row in df_results.iterrows():
        consumption = row['Расход (л)']
        consumption_class = "positive" if consumption < 0 else "negative"  # Отрицательный расход = заправили больше
        
        fuelings = row['Заправлено (л)']
        fuelings_display = f"+{fuelings}" if fuelings > 0 else f"{fuelings}"
        fuelings_color = "#28a745" if fuelings > 0 else "#666"
        ...
        html += f"""
                        <tr>
                            ...
                            <td style='color:{fuelings_color}; font-weight:bold;'>{fuelings_display} л</td>
                            <td>{row['Топливо конец (л)']} л</td>
                            <td class='fuel-change'>{consumption} л</td>
                            <td>
    ...
    total_fuelings = df_results['Заправлено (л)'].sum()
    total_consumption = df_results['Расход (л)'].sum()
    ...
    html += f"""
                ...
                <div class='stats'>
                    ...
                    <div class='stat-card'>
                        <div class='stat-value' style='color: #28a745'>+{total_fuelings:.1f} л</div>
                        <div class='stat-label'>Заправлено</div>
                    </div>
                    <div class='stat-card'>
                    ...
                    <div class='stat-card'>
                        <div class='stat-value' style='color: #dc3545'>{total_consumption:.1f} л</div>
